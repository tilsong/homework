...
public class OpenBank {
	...
	//예금 계좌 개설
	public List<Bank> openBank(List<Bank> list, int index) {
		int temp;
		if(index==-1) {//신규
	...
	//대출 계좌 개설
	public List<Bank> openLoanBank(List<Bank> list, int index) {
		int temp;
		if(index==-1) {//신규
			list.add(new Bank());//리스트에 새로운 Bank인스턴스가 생성됨
			temp=list.size()-1;//해당 인덱스에 정보를 입력함
	...
	//고객정보입력
	public List<Bank> customerInfo(List<Bank> list, int temp) {
